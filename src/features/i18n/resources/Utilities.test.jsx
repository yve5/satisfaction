...
const dictionaryEn = {
  'John got $pineapples pineapples, $oranges bananas and $oranges oranges.':
    'John got $pineapples pineapples, $oranges bananas and $oranges oranges.',
  'Orange is a French telecommunications company.': 'Orange is a French telecommunications company.',
...
const dictionaryFr = {
  ...
  'Orange is a French telecommunications company.': 'Orange est une entreprise française de télécommunications.',
...
describe('tsl', () => {
  ...
  it('should translate non-translated text', () => {
    expect(tools.tsl(dictionaryFr, 'Orange is a French company.')).toEqual('Orange is a French company.');
  });

  it('should translate translated text', () => {
    expect(tools.tsl(dictionaryFr, 'Orange is a French telecommunications company.')).toEqual(
      'Orange est une entreprise française de télécommunications.'
    );
  });

  it('should handle text with args in text, but not in params', () => {
    expect(tools.tsl(dictionaryFr, 'John got $pineapples pineapples, $oranges bananas and $oranges oranges.')).toEqual(
      'John a reçu $pineapples ananas, $oranges bananes et $oranges oranges.'
    );
  });

  it('should handle text with args in params, but not in text', () => {
    expect(tools.tsl(dictionaryFr, 'Orange is a French telecommunications company.', { $oranges: 7 })).toEqual(
      'Orange est une entreprise française de télécommunications.'
    );
  });
  ...
  it('should handle text with args', () => {
    expect(
      tools.tsl(dictionaryFr, 'John got $pineapples pineapples, $oranges bananas and $oranges oranges.', {
        $oranges: 7,
      })
  ...
  it('should handle untranslated text with args', () => {
    expect(
      tools.tsl(dictionaryFr, 'ROBERT got $pineapples pineapples, $oranges bananas and $oranges oranges.', {
        $oranges: 17,
        $pineapples: 42,
      })
  ...
  it('should handle text with non-object args', () => {
    expect(
      tools.tsl(dictionaryFr, 'John got $pineapples pineapples, $oranges bananas and $oranges oranges.', 'Hello World')
    ).toEqual('John a reçu $pineapples ananas, $oranges bananes et $oranges oranges.');
  });
  ...
  it('should handle untranslated text with date args', () => {
    expect(
      tools.tsl(dictionaryFr, 'Update of $update, created at $start by $guest', {
        $guest: 'John',
  ...
  it('should handle untranslated text with date, format and lang args', () => {
    ...
    ).toEqual('Mise à jour le 14 mars 2020, créé le 14/05/2017, 17:51 par John');
  });
...
describe('checkMissigTerms', () => {
  it('should detect fake missing terms', () => {
    ...
    expect(
      tools.checkMissingTerms({ Actions: 'Actions', ...dictionaryEn, Assistance: 'Assistance' }, dictionaryFr)
    ...
    expect(tools.checkMissingTerms({ ...dictionaryEn }, { ...dictionaryFr, ID: 'ID' })).toEqual(['ID']);
  });
...
describe('checkTermOrder', () => {
  it('should check fake term order', () => {
    expect(tools.checkTermOrder(dictionaryEn)).toEqual(tools.convertCollectionToArray(dictionaryEn));

    expect(tools.checkTermOrder(dictionaryFr)).toEqual(tools.convertCollectionToArray(dictionaryFr));

    expect(tools.checkTermOrder({ ID: 'ID', Actions: 'Actions', ...dictionaryEn, Assistance: 'Assistance' })).toEqual([
      'Actions',
      'Assistance',
      'ID',
    ...
    expect(tools.checkTermOrder({ ID: 'ID', Actions: 'Actions', ...dictionaryFr, Zip: 'Zip' })).toEqual([
      'Actions',
      'ID',
  ...
  it('should check term order', () => {
    expect(tools.checkTermOrder(en)).toEqual(tools.convertCollectionToArray(en));
    expect(tools.checkTermOrder(fr)).toEqual(tools.convertCollectionToArray(fr));
  });
});
...
describe('formatDate', () => {
  ...
  it('should handle a plain date in french', () => {
    ...
    expect(tools.formatDate(example, 'PPp', 'fr')).toEqual('14 mars 2020, 13:37');
  ...
  it('should handle a string date', () => {
    ...
    expect(tools.formatDate(stringDate, 'PPp')).toEqual('Sep 10, 2019, 12:00 AM');
  ...
  it('should handle a string date in french', () => {
    ...
    expect(tools.formatDate(stringDate, 'PPp', 'fr')).toEqual('10 sept. 2019, 00:00');
    expect(tools.formatDate(stringDate, 'Pp', 'fr')).toEqual('10/09/2019, 00:00');
  ...
  it('should handle a string datetime', () => {
    ...
    expect(tools.formatDate(stringDatetime, 'PPp')).toEqual('Jan 13, 2020, 12:44 PM');
    expect(tools.formatDate(stringDatetime, 'Pp')).toEqual('01/13/2020, 12:44 PM');
  ...
  it('should handle a string datetime in french', () => {
    ...
    expect(tools.formatDate(stringDatetime, 'PP', 'fr')).toEqual('13 janv. 2020');
    expect(tools.formatDate(stringDatetime, 'PPp', 'fr')).toEqual('13 janv. 2020, 12:44');
    expect(tools.formatDate(stringDatetime, 'Pp', 'fr')).toEqual('13/01/2020, 12:44');
    expect(tools.formatDate(stringDatetime, 'y', 'fr')).toEqual('2020');
    expect(tools.formatDate(stringDatetime, 'fromNow', 'fr')).toMatch(/(il y a)/i);
  });
});
